...
console.log("Consultando comprobantes de pago...");

try {
  const snapshot = await getDocs(collection(db, "comprobantes_pago"));
  console.log(`Total documentos encontrados: ${snapshot.size}`);
  snapshot.forEach(doc => {
    console.log(`ID: ${doc.id}`);
    const data = doc.data();
    console.log(`id_usuario: ${data.id_usuario}, uidUsuario: ${data.uidUsuario}, user: ${data.user}, id_solicitud: ${data.id_solicitud}`);
    console.log(`fecha: ${data.fecha_emision}, monto: ${data.monto_total}`);
  });
  process.exit(0);
} catch (error) {
